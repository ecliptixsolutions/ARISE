...
function Page() {
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () =>
      ((
        await (supabase as any).from("orders").select("*").order("created_at", { ascending: false })
      ).data ?? []) as any[],
  });
  useEffect(() => {
  ...
  const filtered = data.filter((o) => {
    const haystack =
      `${o.order_number} ${o.customer_name} ${o.customer_email} ${o.customer_mobile} ${o.tracking_id}`.toLowerCase();
      ...
      <div className="mt-4 flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search order, customer, email, mobile, tracking..."
            ...
            {!isLoading && filtered.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-8 text-center text-sm text-muted-foreground">
                  {data.length ? "No matching orders found." : "No orders found."}
                </td>
              </tr>
            )}
          </tbody>
