...
function Page() {
  ...
  const canRead = hasPermission(auth, "orders");
  const qc = useQueryClient();
  const { data = [], isLoading } = useQuery({
    queryKey: ["admin-orders"],
    enabled: canRead,
    queryFn: async () => {
      const { data, error } = await apiGet<any[]>("/api/orders");
  ...
  if (!canRead) return <div className="rounded-2xl border border-border bg-card p-6 text-sm text-muted-foreground">Access denied.</div>;
  ...
  function exportCsv() {
    ...
    const csv = [cols.join(","), ...filtered.map(o => cols.map(c => `"${String((o as any)[c] ?? "").replace(/"/g, '""')}"`).join(","))].join("\n");
    ...
    <div>
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{filtered.length} of {(data as any[]).length} orders</p>
        </div>
      ...
      <div className="mt-4 flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search order, customer, tracking..." className="w-full rounded-lg border border-border bg-card py-2.5 pl-10 pr-3 text-sm" /></div>
      ...
      <div className="mt-4 overflow-x-auto rounded-2xl border border-border bg-card">
        <table className="w-full text-sm">
          <thead className="bg-surface text-left text-xs uppercase tracking-wider text-muted-foreground">
            <tr>{["Order","Customer","Product/Service","Amount","Payment","Status","Date",""].map(h => <th key={h} className="px-4 py-3">{h}</th>)}</tr>
          </thead>
          <tbody>
            {filtered.map((o: any) => (
            ...
            {!isLoading && !filtered.length && <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-muted-foreground">No orders found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
