      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <section className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="mt-3 divide-y divide-border">
            ...
            {data.items.length === 0 && (
              <div className="py-6 text-sm text-muted-foreground">{o.service_name}</div>
            )}
          </div>
        </section>
        <aside className="grid gap-4">
          <Card
            title="Customer"
            rows={[o.customer_name, o.customer_email, o.customer_mobile, o.shipping_address]}
          />
          <Card
            title="Payment"
            rows={[
          ...
          <Card
            ...
            rows={[o.tracking_id, o.status.replaceAll("_", " "), o.fulfillment_note]}
          />
        </aside>
      </div>
          ...
          {data.events.map((e: any) => (
            <li key={e.id} className="text-sm">
              ...
              <div className="text-xs text-muted-foreground">
                {e.message} · {new Date(e.created_at).toLocaleString()}
              </div>
            </li>
          ))}
